...
def crear_grafico_pareto(df):
    ...
    ax1.bar(df["Descriptor priorizado"], df["Frecuencia"])
    ax1.set_ylabel("Frecuencia")
    ax1.set_xlabel("Descriptor priorizado")
    ...
    plt.title("Diagrama de Pareto de las problemáticas priorizadas")
    plt.tight_layout()
...
def tabla_priorizados_pdf(df, styles):
    ...
    table = Table(
        data,
        colWidths=[3.6 * inch, 1.0 * inch, 1.1 * inch, 1.4 * inch],
        repeatRows=1
    )
...
def generar_pdf(datos, df, texto_resultados):
    ...
    story.append(Spacer(1, 0.15 * inch))
    story.append(Paragraph("Figura 1. Diagrama de Pareto de las problemáticas priorizadas.", styles["Parrafo"]))
